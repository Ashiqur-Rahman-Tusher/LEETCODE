...
pair<int ,int> max_min_div(int arr[],int l,int h){
    ...
    if(left_res.first==right_res.first){
        res.second = max(left_res.second,right_res.second);

    }
    else if(res.first==left_res.first){
        res.second = max(left_res.second,right_res.first);
    }
    else{
        res.second = max(right_res.second,left_res.first);
    }
...
int main(){
    ...
    cout<<"max : "<<res_div.first<<endl;
    cout<<"max 2 : "<< res_div.second<<endl;
